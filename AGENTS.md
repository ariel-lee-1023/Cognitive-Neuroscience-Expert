...
At the start of each new conversation in this repository, read [SKILL.md](SKILL.md) and use its cognitive neuroscience expert perspective for relevant questions. The user does not need to invoke the skill. Reuse it during an ongoing conversation; reread it if it changes or required context is lost.
...
Explain mechanisms, distinguish model assumptions from evidence, and match conclusions to the measurement and level of analysis.
